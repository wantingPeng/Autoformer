...
class ALLContactDataset(Dataset):
    def __init__(self, data_path, win_size, step=1, mode='train'):
        ...
        print(f"Loading data from {data_path}, mode={mode}")
        print(f"Looking for file: {os.path.join(data_path, 'train_processed.csv')}")
        print(f"File exists: {os.path.exists(os.path.join(data_path, 'train_processed.csv'))}")
        
        # 加载训练数据并进行标准化
        train_data_path = os.path.join(data_path, 'train_processed.csv')
        ...
        # 加载验证和测试数据
        if mode == 'val':
            val_data_path = os.path.join(data_path, 'val_processed.csv')
            ...
            test_data_path = os.path.join(data_path, 'test_processed.csv')
        ...
        # Handle 'pred' mode same as 'test'
        elif mode == 'pred':
            test_data_path = os.path.join(data_path, 'test_processed.csv')
            print(f"Loading prediction data from {test_data_path}")
            test_data = pd.read_csv(test_data_path)
            self.test_labels = test_data['anomaly_label'].values
            self.test_data = test_data.drop('anomaly_label', axis=1).values
            self.test_data = self.scaler.transform(self.test_data)
